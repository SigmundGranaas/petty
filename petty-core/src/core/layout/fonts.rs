...
use crate::traits::{FontProvider, FontQuery, SharedFontData};
...
impl SharedFontLibrary {
    ...
    pub fn load_fallback_font(&self) {
        // Try multiple paths for fonts - supports both workspace root and crate-specific locations
        let font_paths = [
            "assets/fonts/Helvetica.ttf",
            "../assets/fonts/Helvetica.ttf",  // When running from petty-core subdirectory
        ];
    ...
    pub fn resolve_font_data(&self, style: &ComputedStyle) -> Result<FontData, crate::traits::FontError> {
        ...
        {
            Err(crate::traits::FontError::NotFound {
    ...
    fn resolve_from_fontdb(
    ...
    ) -> Result<FontData, crate::traits::FontError> {
        ...
        let id = {
            let db = self.db.read()
                .map_err(|_| crate::traits::FontError::LoadFailed {
            ...
            db.query(&query).or_else(|| {
                ...
                crate::traits::FontError::NotFound {
                    family: family.to_string(),
                    weight: weight.clone(),
                    style: font_style.clone(),
                }
        ...
        let face_info = db.face(id)
            .ok_or_else(|| crate::traits::FontError::NotFound {
                family: family.to_string(),
                weight: weight.clone(),
                style: font_style.clone(),
            })?;
        ...
        let font_bytes: SharedFontData = match &face_info.source {
            ...
            fontdb::Source::File(path) => {
                ...
                Arc::new(std::fs::read(path)
                    .map_err(|e| crate::traits::FontError::LoadFailed {
            ...
            fontdb::Source::File(path) => {
                return Err(crate::traits::FontError::LoadFailed {
                    path: path.display().to_string(),
                    message: "file system not available on WASM".to_string(),
                });
            }
            _ => return Err(crate::traits::FontError::InvalidData(
...
mod tests {
    use super::*;
    use crate::traits::InMemoryFontProvider;
