// FILE: /home/sigmund/RustroverProjects/petty/src/parser/xslt/executor_handlers/call_template.rs
use crate::parser::xslt::datasource::DataSourceNode;
use crate::parser::xslt::xpath::{self};
use crate::parser::xslt::ast::WithParam;
    ...
    if let Some(template) = executor.stylesheet.named_templates.get(name) {
        ...
        for defined_param in &template_clone.params {
            let param_value = if let Some(passed_value) = passed_params.get(&defined_param.name) {
                ...
                xpath::evaluate(default_expr, &default_e_ctx)?
            } else {
                xpath::XPathValue::String("".to_string())
            };
