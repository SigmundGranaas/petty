...
pub fn evaluate_function<'a, 'd, N: DataSourceNode<'a>>(
    ...
    match name {
        ...
        "node" | "comment" | "processing-instruction" => Err(XPathError::FunctionError { function: name.to_string(), message: "This is a node-test, not a function.".to_string() }),
        _ => Err(XPathError::FunctionError { function: name.to_string(), message: "Unknown XPath function".to_string() }),
    }
}
...
fn func_id<'a, 'd, N: DataSourceNode<'a>>(
    ...
    if args.len() != 1 {
        return Err(XPathError::FunctionError { function: "id()".to_string(), message: "Expected 1 argument".to_string() });
    }
...
fn func_key<'a, 'd, N: DataSourceNode<'a>>(
    ...
    if args.len() != 2 {
        return Err(XPathError::FunctionError { function: "key()".to_string(), message: "Expected 2 arguments".to_string() });
    }
...
fn func_string<'a, 'd, N: DataSourceNode<'a>>(
    ...
    if args.len() > 1 {
        return Err(XPathError::FunctionError { function: "string()".to_string(), message: "Expected 0 or 1 arguments".to_string() });
    }
...
fn func_count<'a, N: DataSourceNode<'a>>(mut args: Vec<XPathValue<N>>) -> Result<XPathValue<N>, XPathError> {
    if args.len() != 1 {
        return Err(XPathError::FunctionError { function: "count()".to_string(), message: "Expected 1 argument".to_string() });
    }
    let count = match args.remove(0) {
        XPathValue::NodeSet(nodes) => nodes.len() as f64,
        v => return Err(XPathError::TypeError(format!("count() argument must be a node-set, got {:?}", v))),
    };
    Ok(XPathValue::Number(count))
}

fn func_position<'a, 'd, N: DataSourceNode<'a>>(args: Vec<XPathValue<N>>, e_ctx: &EvaluationContext<'a, 'd, N>) -> Result<XPathValue<N>, XPathError> {
    if !args.is_empty() {
        return Err(XPathError::FunctionError { function: "position()".to_string(), message: "Expected 0 arguments".to_string() });
    }
    Ok(XPathValue::Number(e_ctx.context_position as f64))
}

fn func_last<'a, 'd, N: DataSourceNode<'a>>(args: Vec<XPathValue<N>>, e_ctx: &EvaluationContext<'a, 'd, N>) -> Result<XPathValue<N>, XPathError> {
    if !args.is_empty() {
        return Err(XPathError::FunctionError { function: "last()".to_string(), message: "Expected 0 arguments".to_string() });
    }
...
fn func_local_name<'a, 'd, N: DataSourceNode<'a>>(
    ...
    if args.len() > 1 {
        return Err(XPathError::FunctionError { function: "local-name()".to_string(), message: "Expected 0 or 1 arguments".to_string() });
    }
    let node = if args.is_empty() {
        ...
        match args.remove(0) {
            ...
            v => return Err(XPathError::TypeError(format!("local-name() argument must be a node-set, got {:?}", v))),
        }
    };
...
fn func_name<'a, 'd, N: DataSourceNode<'a>>(
    ...
    if args.len() > 1 {
        return Err(XPathError::FunctionError { function: "name()".to_string(), message: "Expected 0 or 1 arguments".to_string() });
    }
    let node = if args.is_empty() {
        Some(e_ctx.context_node)
    } else {
        match args.remove(0) {
            XPathValue::NodeSet(nodes) => nodes.first().copied(),
            v => return Err(XPathError::TypeError(format!("name() argument must be a node-set, got {:?}", v))),
        }
    };
...
fn func_concat<'a, N: DataSourceNode<'a>>(args: Vec<XPathValue<N>>) -> Result<XPathValue<N>, XPathError> {
    if args.len() < 2 {
        return Err(XPathError::FunctionError { function: "concat()".to_string(), message: "Expected at least 2 arguments".to_string() });
    }
    let result = args.iter().map(|v| v.to_string()).collect::<String>();
    Ok(XPathValue::String(result))
}

fn func_starts_with<'a, N: DataSourceNode<'a>>(mut args: Vec<XPathValue<N>>) -> Result<XPathValue<N>, XPathError> {
    if args.len() != 2 {
        return Err(XPathError::FunctionError { function: "starts-with()".to_string(), message: "Expected 2 arguments".to_string() });
    }
...
fn func_contains<'a, N: DataSourceNode<'a>>(mut args: Vec<XPathValue<N>>) -> Result<XPathValue<N>, XPathError> {
    if args.len() != 2 {
        return Err(XPathError::FunctionError { function: "contains()".to_string(), message: "Expected 2 arguments".to_string() });
    }
    let s2 = args.remove(1).to_string();
    let s1 = args.remove(0).to_string();
...
fn func_substring_before<'a, N: DataSourceNode<'a>>(mut args: Vec<XPathValue<N>>) -> Result<XPathValue<N>, XPathError> {
    if args.len() != 2 {
        return Err(XPathError::FunctionError { function: "substring-before()".to_string(), message: "Expected 2 arguments".to_string() });
    }
    let s2 = args.remove(1).to_string();
    let s1 = args.remove(0).to_string();
...
fn func_substring_after<'a, N: DataSourceNode<'a>>(mut args: Vec<XPathValue<N>>) -> Result<XPathValue<N>, XPathError> {
    if args.len() != 2 {
        return Err(XPathError::FunctionError { function: "substring-after()".to_string(), message: "Expected 2 arguments".to_string() });
    }
    let s2 = args.remove(1).to_string();
    let s1 = args.remove(0).to_string();
    if let Some(index) = s1.find(&s2) {
...
fn func_substring<'a, N: DataSourceNode<'a>>(mut args: Vec<XPathValue<N>>) -> Result<XPathValue<N>, XPathError> {
    if !(2..=3).contains(&args.len()) {
        return Err(XPathError::FunctionError { function: "substring()".to_string(), message: "Expected 2 or 3 arguments".to_string() });
    }
...
fn func_string_length<'a, 'd, N: DataSourceNode<'a>>(
    ...
    if args.len() > 1 {
        return Err(XPathError::FunctionError { function: "string-length()".to_string(), message: "Expected 0 or 1 arguments".to_string() });
    }
    let s = if args.is_empty() {
        e_ctx.context_node.string_value()
    } else {
...
fn func_normalize_space<'a, 'd, N: DataSourceNode<'a>>(
    ...
    if args.len() > 1 {
        return Err(XPathError::FunctionError { function: "normalize-space()".to_string(), message: "Expected 0 or 1 arguments".to_string() });
    }
    let s = if args.is_empty() {
        e_ctx.context_node.string_value()
    } else {
...
fn func_translate<'a, N: DataSourceNode<'a>>(mut args: Vec<XPathValue<N>>) -> Result<XPathValue<N>, XPathError> {
    if args.len() != 3 {
        return Err(XPathError::FunctionError { function: "translate()".to_string(), message: "Expected 3 arguments".to_string() });
    }
...
fn func_not<'a, N: DataSourceNode<'a>>(mut args: Vec<XPathValue<N>>) -> Result<XPathValue<N>, XPathError> {
    if args.len() != 1 {
        return Err(XPathError::FunctionError { function: "not()".to_string(), message: "Expected 1 argument".to_string() });
    }
    Ok(XPathValue::Boolean(!args.remove(0).to_bool()))
}

fn func_true<'a, N: DataSourceNode<'a>>(args: Vec<XPathValue<N>>) -> Result<XPathValue<N>, XPathError> {
    if !args.is_empty() {
        return Err(XPathError::FunctionError { function: "true()".to_string(), message: "Expected 0 arguments".to_string() });
    }
    Ok(XPathValue::Boolean(true))
}

fn func_false<'a, N: DataSourceNode<'a>>(args: Vec<XPathValue<N>>) -> Result<XPathValue<N>, XPathError> {
    if !args.is_empty() {
        return Err(XPathError::FunctionError { function: "false()".to_string(), message: "Expected 0 arguments".to_string() });
    }
...
fn func_lang<'a, 'd, N: DataSourceNode<'a>>(
    ...
    if args.len() != 1 {
        return Err(XPathError::FunctionError { function: "lang()".to_string(), message: "Expected 1 argument".to_string() });
    }
    ...
    while let Some(node) = current {
        for attr in node.attributes() {
            if let Some(name) = attr.name()
                && name.prefix == Some("xml") && name.local_part == "lang" {
...
fn func_sum<'a, N: DataSourceNode<'a>>(mut args: Vec<XPathValue<N>>) -> Result<XPathValue<N>, XPathError> {
    if args.len() != 1 {
        return Err(XPathError::FunctionError { function: "sum()".to_string(), message: "Expected 1 argument".to_string() });
    }
    let sum = match args.remove(0) {
        ...
        v => return Err(XPathError::TypeError(format!("sum() argument must be a node-set, got {:?}", v))),
    };
    Ok(XPathValue::Number(sum))
}

fn func_floor<'a, N: DataSourceNode<'a>>(mut args: Vec<XPathValue<N>>) -> Result<XPathValue<N>, XPathError> {
    if args.len() != 1 {
        return Err(XPathError::FunctionError { function: "floor()".to_string(), message: "Expected 1 argument".to_string() });
    }
    Ok(XPathValue::Number(args.remove(0).to_number().floor()))
}

fn func_ceiling<'a, N: DataSourceNode<'a>>(mut args: Vec<XPathValue<N>>) -> Result<XPathValue<N>, XPathError> {
    if args.len() != 1 {
        return Err(XPathError::FunctionError { function: "ceiling()".to_string(), message: "Expected 1 argument".to_string() });
    }
    Ok(XPathValue::Number(args.remove(0).to_number().ceil()))
}

fn func_round<'a, N: DataSourceNode<'a>>(mut args: Vec<XPathValue<N>>) -> Result<XPathValue<N>, XPathError> {
    if args.len() != 1 {
        return Err(XPathError::FunctionError { function: "round()".to_string(), message: "Expected 1 argument".to_string() });
    }
...
mod tests {
    use super::*;
    use crate::datasource::tests::{create_test_tree, MockNode, MockTree};
