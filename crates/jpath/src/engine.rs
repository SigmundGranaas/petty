    ...
    match expr {
        ...
        Expression::FunctionCall { name, args } => {
            let function = e_ctx
                .functions
                .get(name)
                .ok_or_else(|| JPathError::TemplateRender(format!("Unknown function: {}", name)))?;
...
pub fn evaluate_as_bool(expr: &Expression, e_ctx: &EvaluationContext) -> Result<bool, JPathError> {
