...
impl<'s, 'd> TemplateExecutor<'s, 'd> {
    ...
    pub fn build_tree(
        ...
        context: &Value,
    ) -> Result<Vec<IRNode>, JsonTemplateError> {
    ...
    fn gather_styles(
        ...
        context: &Value,
        loop_pos: Option<usize>,
    ) -> Result<Vec<Arc<ElementStyle>>, JsonTemplateError> {
    ...
    fn render_string(
        ...
        context: &Value,
        loop_pos: Option<usize>,
    ) -> Result<String, JsonTemplateError> {
        match compiled_str {
                ...
                for part in parts {
                    match part {
                        ...
                        ExpressionPart::Dynamic(expression) => {
                            let s = petty_jpath::evaluate_as_string(expression, &e_ctx)?;
                            result.push_str(&s);
                        }
                    }
                }
...
// --- Helper conversion functions (formerly TryFrom impls) ---
...
fn ir_node_to_table_cell(node: IRNode) -> Result<TableCell, JsonTemplateError> {
