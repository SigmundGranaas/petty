...
pub(crate) fn get_owned_attributes(e: &BytesStart) -> Result<OwnedAttributes, XsltError> {
    e.attributes()
        .map(|attr_res| {
            let attr = attr_res?;
...
pub(crate) fn parse_avt(builder: &mut CompilerBuilder, text: &str) -> Result<AttributeValueTemplate, XsltError> {
    ...
    for (start, _part) in text.match_indices('{') {
        ...
        let end = text[start..]
            .find(end_marker)
            .ok_or_else(|| XsltError::TemplateParse("Unclosed { expression in AVT".to_string()))?;
...
pub(super) fn get_attr_owned_optional(
    ...
    if let Some((_key, value)) = attributes.iter().find(|(k, _v)| k.as_slice() == name) {
        Ok(Some(
            String::from_utf8(value.clone()).map_err(XsltError::Utf8)?,
        ))
    } else {
        Ok(None)
    }
