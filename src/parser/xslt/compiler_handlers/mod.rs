// FILE: /home/sigmund/RustroverProjects/petty/src/parser/xslt/compiler_handlers/mod.rs
...
impl CompilerBuilder {
    ...
    pub(crate) fn handle_copy_end(
        ...
        let location = crate::parser::xslt::util::get_line_col_from_pos(source, pos).into();
        if let BuilderState::InstructionBody(attrs) = current_state {
        ...
        match current_state {
            BuilderState::Attribute(prop) => {
                ...
                if let Some(BuilderState::AttributeSet { style, .. }) = self.state_stack.last_mut() {
                    crate::parser::style::apply_style_property(style, &prop, &value)?;
                }
            }
    ...
    pub(crate) fn handle_attribute_set_end(&mut self, current_state: BuilderState) -> Result<(), ParseError> {
