...
use super::util::{
    get_attr_owned_optional, get_attr_owned_required, get_attr_required, parse_table_columns,
    OwnedAttributes,
};
...
impl<'a> TreeBuilder<'a> {
    ...
    fn push_block_to_parent(&mut self, node: IRNode) {
        match self.node_stack.last_mut() {
            ...
            Some(IRNode::Table { body, .. }) => match body.rows.last_mut() {
                ...
                None => log::warn!("Attempted to add node to a table body with no rows."),
            },
    ...
    /// Recursively parses XML nodes and builds the `IRNode` tree.
    fn parse_nodes(
        &mut self,
            ...
            match reader.read_event_into(&mut buf)? {
                ...
                XmlEvent::End(e) => {
                    if self.handle_end_tag(e.name())? {
                        return Ok(()); // Reached the end tag for this recursive call
                    }
                }
    ...
    fn handle_start_tag(
        ...
        // Collect attributes once, as they might be needed by multiple handlers.
        let attributes = e
            .attributes()
            .map(|a| a.map(|attr| (attr.key.as_ref().to_vec(), attr.value.into_owned())))
            .collect::<Result<OwnedAttributes, _>>()?;
        ...
        match tag_name.as_ref() {
            ...
            b"xsl:for-each" => {
                ...
                for item_context in items {
                    self.parse_nodes(reader, item_context)?;
                }
            }
            b"xsl:if" => {
                let test = get_attr_owned_required(&attributes, b"test", b"xsl:if")?;
                let results = xpath::select(context, &test);
                if !results.is_empty() && results.iter().all(|v| !v.is_null()) {
                    self.parse_nodes(reader, context)?;
                } else {
                    reader.read_to_end_into(tag_name, &mut vec![])?; // Skip content
                }
            }

            // --- Block Elements ---
            b"container" | b"list" | b"list-item" | b"flex-container" => {
                let style = get_attr_owned_optional(&attributes, b"style")?;
            ...
            b"strong" | b"b" | b"em" | b"i" => {
                let style_name = if matches!(tag_name.as_ref(), b"strong" | b"b") {
                    "bold"
                } else {
                    "italic"
                };
            ...
            b"header" => self.handle_header_start(reader, context)?,
            b"tbody" => self.parse_nodes(reader, context)?, // tbody is implicit
            b"row" => self.handle_row_start(reader, context)?,
            b"cell" => self.handle_cell_start(&attributes, reader, context)?,

            // --- Structural / Ignored ---
            b"petty-wrapper" => self.parse_nodes(reader, context)?, // Descend into dummy wrapper
            b"columns" | b"column" => {
                reader.read_to_end_into(tag_name, &mut vec![])?;
            }
            _ => {
                // Unknown block tag, just parse children
                log::warn!("Ignoring unknown start tag: {}", String::from_utf8_lossy(tag_name.as_ref()));
                self.parse_nodes(reader, context)?;
            }
        }
        Ok(())
    ...
    fn handle_end_tag(&mut self, qname: QName) -> Result<bool, PipelineError> {
        ...
        match tag_name {
            b"container" | b"list" | b"list-item" | b"text" | b"table" | b"flex-container" => {
                if let Some(node) = self.node_stack.pop() {
                    self.push_block_to_parent(node);
                }
            }
            b"header" | b"row" | b"cell" => { /* State managed by start handlers */ }
...
impl<'a> TreeBuilder<'a> {
    fn handle_table_start(
        ...
        let style = get_attr_owned_optional(attributes, b"style")?;
        // Need to read ahead to find the <columns> definition.
        let inner_xml = super::capture_inner_xml(reader, QName(b"table"))?;
        ...
        // Now parse the table content for real.
        ...
        match self.node_stack.last_mut() {
            Some(IRNode::Table { header, body, .. }) => {
                if let Some(h) = header {
                    // Check if we are still filling the header
                    // This is a simplification; assumes header content comes first.
                    // A more robust solution might track a "current section" state.
                    let is_in_header = body.rows.is_empty();
                    if is_in_header {
                        h.rows.push(new_row);
                    } else {
    ...
    fn handle_cell_start(
        ...
        match self.node_stack.last_mut() {
            Some(IRNode::Table { header, body, .. }) => {
                let is_in_header = header.is_some() && body.rows.is_empty();
                let row = if is_in_header {
                    header.as_mut().unwrap().rows.last_mut()
                } else {
                    body.rows.last_mut()
                };
                if let Some(r) = row {
