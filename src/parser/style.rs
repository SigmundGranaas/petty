...
//! `nom` parsers from the `style_parsers` module to interpret strings from template
...
use crate::parser::style_parsers::{run_parser, parse_dimension, parse_length};
...
impl<'de> Deserialize<'de> for FontWeight {
    ...
    where D: Deserializer<'de>,
    {
        #[derive(Deserialize)]
        #[serde(untagged)]
        enum FontWeightDef { Str(String), Num(u16), }
...
pub fn apply_style_property(style: &mut ElementStyle, attr_name: &str, value: &str) -> Result<(), ParseError> {
    match attr_name {
        ...
        "color" => style.color = Some(run_parser(super::style_parsers::parse_color, value)?),
        "background-color" => style.background_color = Some(run_parser(super::style_parsers::parse_color, value)?),
        "border" => style.border = Some(run_parser(super::style_parsers::parse_border, value)?),
        "border-top" => style.border_top = Some(run_parser(super::style_parsers::parse_border, value)?),
        "border-bottom" => style.border_bottom = Some(run_parser(super::style_parsers::parse_border, value)?),
        "margin" => style.margin = Some(super::style_parsers::parse_shorthand_margins(value)?),
        ...
        "padding" => style.padding = Some(super::style_parsers::parse_shorthand_margins(value)?),
