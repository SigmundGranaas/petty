...
pub fn layout_list_item(
    ...
    // A real text engine would be needed to get accurate bullet metrics
    let bullet_width = style.font_size * 0.6 * 2.0; // "• " approx
    ...
    for child in children.iter().rev() {
        ...
        // Indent content to the right of the bullet.
        indented_style.padding.left += 20.0;
        work_stack.push((WorkItem::Node(child.clone()), indented_style));
    }
    // The bullet is the only element produced by this function directly.
    (vec![bullet], 0.0, None)
}
...
mod tests {
    ...
    fn test_layout_block_pushes_children_to_stack() {
        ...
        let mut children = [
            IRNode::Paragraph { style_name: Some("p1".to_string()), children: vec![] },
            IRNode::Paragraph { style_name: Some("p2".to_string()), children: vec![] },
        ];
    ...
    fn test_layout_list_item_creates_bullet_and_indents() {
        ...
        assert_eq!(elements[0].x, original_padding);
        ...
        assert_eq!(indented_style.padding.left, original_padding + 20.0);
    }
}