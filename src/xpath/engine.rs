// FILE: /home/sigmund/RustroverProjects/petty/src/xpath/engine.rs
//! The evaluation engine for executing a parsed expression AST.
...
pub fn evaluate(expr: &Expression, e_ctx: &EvaluationContext) -> Result<Value, ParseError> {
    match expr {
        ...
        Expression::Selection(sel) => {
            // A selection can return multiple nodes, but in most expression contexts,
...
/// "Truthiness" rules:
/// - `false`, `null`, `0`, `""` are false.
/// - Empty arrays/objects are false.
...
pub fn select<'a>(
    ...
    match sel {
        ...
        Selection::JsonPointer(path) => context.pointer(path).map_or(vec![], |v| vec![v]),
