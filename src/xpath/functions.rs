// FILE: /home/sigmund/RustroverProjects/petty/src/xpath/functions.rs
//! Defines the registry and built-in implementations for XPath functions.
use super::engine::EvaluationContext;
use serde_json::{json, Value};
use std::collections::HashMap;

/// The signature for a custom XPath function implementation.
/// It receives the evaluation context and a vector of evaluated arguments,
/// and returns a single `Value`.
pub type XPathFunction = fn(e_ctx: &EvaluationContext, args: Vec<Value>) -> Value;
...
impl FunctionRegistry {
    ...
    pub fn register(&mut self, name: &str, func: XPathFunction) {
        self.functions.insert(name.to_lowercase(), func);
    }

    /// Finds a function by name (case-insensitive).
    pub fn get(&self, name: &str) -> Option<&XPathFunction> {
...
fn upper(_e_ctx: &EvaluationContext, args: Vec<Value>) -> Value {
    args.get(0).and_then(|v| v.as_str()).map(|s| s.to_uppercase().into()).unwrap_or(Value::Null)
}

fn lower(_e_ctx: &EvaluationContext, args: Vec<Value>) -> Value {
    args.get(0).and_then(|v| v.as_str()).map(|s| s.to_lowercase().into()).unwrap_or(Value::Null)
}
...
fn contains(_e_ctx: &EvaluationContext, args: Vec<Value>) -> Value {
    let haystack = args.get(0).and_then(|v| v.as_str());
