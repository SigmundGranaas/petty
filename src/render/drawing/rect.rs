...
/// Renders background and borders statelessly.
pub(super) fn draw_background_and_borders_stateless(
    ops: &mut Vec<Op>,
    ...
    if let Some(bg_color) = &style.misc.background_color {
        ...
        ops.push(Op::SetFillColor { col: to_pdf_color(bg_color) });
    ...
    if let Some(border) = &style.border.bottom {
        ...
        ops.push(Op::DrawPolygon { polygon: line });
    }
